...
class iter_3():
  def __init__(self, trans_dict):
    ...
    self.iter_folder = os.path.join(os.path.dirname(__file__), 'iter_3/')
  ...
  def check_syllable(self, idx, trans, en_seq, phon_seq):
    ...
    if phon_seq[idx] in self.iter_rules.vowel_set:
      ...
      elif phon_seq[idx] == "AXR" and \
      idx == len(trans.inter_phon_seq)-1 and phon_seq[idx-1] == "_":
        new_syl.en_vowel = pick_items_backwards_from_seq(en_seq, idx, 2)
...
######################################################## PREPROCESS #####################################################################
  def preprocess(self, trans):
    for idx in range(len(trans.inter_phon_seq)):
      ...
      if (trans.inter_phon_seq[idx] == "_" or \
      trans.inter_phon_seq[idx] in self.iter_rules.ini_conso_set or \
      trans.inter_phon_seq[idx] in self.iter_rules.final_conso_set) and \
      trans.inter_en_seq[idx] in self.iter_rules.en_vowels_set and \
      idx < len(trans.inter_phon_seq)-1 :
        left_cond = False
        right_cond = False

        if idx == 0 or \
        (trans.inter_phon_seq[idx-1] in self.iter_rules.ini_conso_set and \
        trans.inter_en_seq[idx-1] in self.iter_rules.en_consos_set):
          left_cond = True
        if idx == len(trans.inter_phon_seq)-1 or \
        ((trans.inter_phon_seq[idx+1] in self.iter_rules.final_conso_set or trans.inter_phon_seq[idx+1] in self.iter_rules.ini_conso_set) and \
        trans.inter_en_seq[idx+1] in self.iter_rules.en_consos_set):
          right_cond = True
...
######################################################## VOWEL MAPPING RULES #####################################################################
  ...
  def map_vowel(self, syl):
    if not self.map_vowel_AXR(syl):
      self.map_vowel_AX(syl)
